...
**细规见**：`bulls-dominant-pickup-route-e-david-discovery.md`（触发/反应四拍/处置工具箱/终局矩阵/分章大纲）。

| 入口 | 情节 | 备注 |
|------|------|------|
| **E1 软发现** | 锁屏、气味、口误、物证 | 发现方式；须留圆谎，忌秒跪 |
| **E2 硬发现** | 撞破房门/会师/吧台复刻 | 发现方式；Marcus 在场 |
| **E3 献妻** | 知情后择路：承认→请求→契约 | **不是触发器**，是终局方向 |
| **E4 驱逐** | 知情后择路：离婚/报警/离开 | **不是触发器**；可真离或假离实囚 |

**粗建议**：经典牛头用 T1+E1→终局 α（Marcus→Elena→David）；一夜爆章用 T0/D→E2；要黑选硬发现+恨着留下；要破裂选 E4。细项以专项文件 §九推荐为准。
