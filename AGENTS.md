...
新故事只需以下两行 YAML，标题用 ASCII 双引号包裹（YAML 要求）：
...
不写 `status` 时，日期前的标记默认为「完结」。`status` 只写三种基态，按「原文是否完结 / 是否有已发布续写」选：

- `断更`：网上原文未完，已发布正文里没有续写。
- `续写`：已发布正文里有续写，且续写尚未完结。不要写 `连载`（旧别名仍能显示成「续写」，但新稿只用 `续写`）。
- `完结`：原文已完结且没有续写；或续写也已经写完。续写写完后暂时也用完结，不要另造标签。

显示日期（`date` / `last_modified_at` / `modified_date` 中最晚的一天）在 **15 天**以内时，标记会改成「更新」，优先级高于上面三种；过期后回到基态。15 天以站点实现为准，不要改成「两周」或 14 天。

更新**已发布**故事的正文时（续写、大段增补、重新上架），在 front matter 增加或改写 `last_modified_at: YYYY-MM-DD` 为今天。首页和文章页会把这一天显示为发布日期，并据此排序。仅改格式、错别字、的/地/得时不要动这一行。不要改文件名里的日期（会改变 URL）。
...
- 不要修改已有 post 的文件名或日期前缀，除非用户明确要求。文章更新用 `last_modified_at`，不要靠改文件名把日期提前。
